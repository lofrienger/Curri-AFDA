...
def shear_x(pil_img, level, im_size):
  level = float_parameter(sample_level(level), 0.3)
  if np.random.uniform() > 0.5:
    level = -level
  return pil_img.transform((im_size, im_size),
                           Image.AFFINE, (1, level, 0, 0, 1, 0),
                           resample=Image.BILINEAR)


def shear_y(pil_img, level, im_size):
  level = float_parameter(sample_level(level), 0.3)
  if np.random.uniform() > 0.5:
    level = -level
  return pil_img.transform((im_size, im_size),
...
def translate_x(pil_img, level, im_size):
  level = int_parameter(sample_level(level), im_size / 3)
...
def translate_y(pil_img, level, im_size):
  level = int_parameter(sample_level(level), im_size / 3)
  if np.random.random() > 0.5:
    level = -level
  return pil_img.transform((im_size, im_size),
